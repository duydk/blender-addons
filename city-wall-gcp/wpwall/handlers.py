...
def depsgraph_update_handler(scene, depsgraph):
    ...
    valid_towers = set(sorted_towers(scene))
    changed = False
    for update in depsgraph.updates:
        obj = getattr(update.id, "original", None) or update.id
        if isinstance(obj, bpy.types.Object) and (obj in valid_waypoints or obj in valid_openings or obj in valid_gates or obj in valid_towers):
